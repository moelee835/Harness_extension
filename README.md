...
| 서비스 | `src/service/AnalyzerService.ts` | ✅ 완료 |
| 설정 | `src/config/AgentConfig.ts` | ✅ 완료 |
| 영속성 | `src/persistence/FileManager.ts` | ✅ 완료 |

### 완료된 기능 요약 (passes=true, 전체 34개)

- **인프라** (F-001~F-003, F-028~F-031): Extension 활성화·비활성화, .vsix 패키징, TypeScript 컴파일, ESLint, esbuild 번들링, npm test
- **UI** (F-004~F-006, F-013, F-032): 메인 패널 열기·싱글톤 포커스, 요구사항 입력 폼, Markdown 변환, Plan View (PLAN.md 단계별 렌더링)
- **에이전트 설정** (F-014~F-016, F-027): 에이전트 타입 선택(claude/gemini/custom), CLI 경로 설정, 추가 플래그, 세션 간 영속성
- **CLI 실행** (F-007, F-017~F-020, F-033~F-034): AgentRunnerFactory 3종 Runner, stdout/stderr 실시간 스트리밍, Init Project 전체 흐름, 오류 표시, 취소 버튼
- **Analyzer** (F-008~F-012): Command / Skill / MCP 스펙 / Hook / Sub-agent .md 파일 생성
- **파일 관리** (F-021~F-026): FileManager CRUD (create/read/update/delete/list), 에이전트 액션 완료 후 파일 목록 UI 표시

### 다음 단계 (v1.0 배포 준비)

1. **통합 테스트 보강** — 실제 VSCode Extension Host 환경에서 E2E 시나리오 검증
2. **`.vsix` 배포 파일 생성** — `npm run package` 후 설치 검증
3. **CHANGELOG.md 작성** — 버전별 변경 이력 정리
4. **사용자 가이드 문서화** — 설치·설정·사용법 스크린샷 포함 정리

기능 요구사항 전체 목록: [`specs/features.json`](specs/features.json) (총 34개 항목, 전체 passes=true)
