...
/**
 * Gemini CLI(`gemini`)를 실행하는 에이전트 실행기.
 * IAgentRunner를 구현하며, AgentRunnerFactory가 agentType='gemini' 설정 시 이 클래스를 반환한다.
 *
 * cliPath가 설정된 경우 해당 경로의 실행 파일을 사용하고,
 * 설정되지 않은 경우(빈 문자열) PATH 환경변수에서 'gemini' 명령을 자동 탐색한다.
 *
 * F-018: AgentRunnerFactory.create()가 gemini 타입에 이 클래스 인스턴스를 반환한다.
 */
export class GeminiCliRunner implements IAgentRunner {
	/**
	 * child_process.spawn에 실제로 전달될 명령 문자열.
	 * cliPath가 빈 문자열이면 'gemini'(PATH 탐색), 그렇지 않으면 cliPath 그대로 사용.
	 */
	 ...
	 * GeminiCliRunner 생성자.
	 *
	 * @param cliPath - Gemini CLI 실행 파일의 절대 경로.
	 *                  빈 문자열이면 PATH에서 'gemini'를 자동 탐색한다.
	 * @param extraArgs - spawn 호출 시 추가로 전달할 CLI 플래그 배열 (예: ['--model', 'gemini-pro'])
	 */
	constructor(cliPath: string, extraArgs: string[]) {
		// cliPath가 공백만 있거나 빈 문자열이면 기본 명령('gemini')을 사용
		this._spawnCommand = cliPath.trim() !== '' ? cliPath : 'gemini';
	 ...
	 *
	 * @returns spawn 명령 문자열 (예: 'gemini' 또는 '/usr/local/bin/gemini')
	 */
	...
	/**
	 * 현재 실행 중인 Gemini CLI 프로세스를 즉시 종료한다.
	...
	/**
	 * Gemini CLI를 child_process.spawn으로 실행하여 주어진 프롬프트를 처리한다.
	 * stdout/stderr 스트리밍은 F-020에서 구현 예정이다.
	 *
	 * @param prompt - Gemini에 전달할 프롬프트 문자열
	...
	public invoke(prompt: string): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			...
			// child_process.spawn으로 Gemini CLI 프로세스 생성
			...
			child.on('close', (code: number | null) => {
				...
				if (code === 0 || code === null) {
					...
					reject(new Error(`Gemini CLI 프로세스가 종료 코드 ${code}로 종료되었습니다.`));
				}
			});
			...
			child.on('error', (err: Error) => {
				...
				this._isRunning = false;
				this._childProcess = null;
				reject(new Error(`Gemini CLI 프로세스 시작 실패: ${err.message}`));
			});
		});
	}
}
