...
- F-024: FileManager.delete()가 지정된 경로의 .md 파일을 삭제한다
  - src/test/extension.test.ts: F-024 테스트 2건 추가
    - delete() 후 파일 접근 불가 확인 (fs.access rejects)
    - 존재하지 않는 파일 경로로 호출 시 에러 발생 검증
  - 53 passing (기존 51 + F-024 2건)
...
- F-022: FileManager.read()가 지정된 경로의 .md 파일을 읽어 반환한다
  - src/test/extension.test.ts: F-022 테스트 2건 추가
    - read()가 파일 내용을 정확히 반환하는지 검증 (임시 파일 생성 후 내용 비교)
    - 존재하지 않는 파일 경로로 호출 시 에러 발생 검증
  - FileManager.read() 구현은 F-021 시점에 이미 완성되어 있었음 (스텁 → 실구현)
  - 49 passing (기존 47 + F-022 2건)
...
- F-011: AnalyzerService.generateHookEntry()로 .claude/settings.json에 훅 설정 항목 생성
  - src/service/AnalyzerService.ts
    - HookCommandEntry, SettingsSchema 내부 타입 추가
    - HookEntryResult 인터페이스 추가 (settingsPath + event + command)
    - generateHookEntry(markdownInput, settingsPath): FileManager.read()+JSON.parse+update() 패턴으로 병합
    - _buildHookEntryPrompt(), _extractHookEvent(), _extractHookCommand() 헬퍼 추가
    - 이스케이프 따옴표 대응: JSON.parse 우선 + 정규식 대체
  - src/test/extension.test.ts: F-011 테스트 2건 추가 (stub runner 기반)
  - 43 passing (기존 41 + F-011 2건)
...
- F-009: AnalyzerService.generateSkill()로 .claude/skills/에 스킬 .md 파일 생성
  - src/service/AnalyzerService.ts
    - SkillAnalysisResult 인터페이스 추가
    - generateSkill(markdownInput, skillsDir): CLI sub-agent 호출 후 stdout 파싱, FileManager로 저장
    - _buildSkillPrompt(): YAML frontmatter + 트리거 조건 + 동작 설명 섹션 포함 프롬프트 구성
    - _extractCommandName() → _extractName()으로 리네임 (공통 헬퍼 통합)
  - src/test/extension.test.ts: F-009 테스트 2건 추가 (stub runner 기반)
  - 39 passing (기존 37 + F-009 2건)
